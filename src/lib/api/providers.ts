...
export const providersApi = {
  async getAll(appId: AppId): Promise<Record<string, Provider>> {
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}`);
    }
  ...
  async getCurrent(appId: AppId): Promise<string> {
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}/current`);
    }
  ...
  async add(
    ...
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}`, {
        method: "POST",
        body: JSON.stringify({ provider, addToLive }),
      });
    }
  ...
  async update(
    ...
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}/${provider.id}`, {
        method: "PUT",
        body: JSON.stringify({ provider, originalId }),
      });
    }
  ...
  async delete(id: string, appId: AppId): Promise<boolean> {
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}/${id}`, {
        method: "DELETE",
      });
    }
  ...
  async switch(id: string, appId: AppId): Promise<SwitchResult> {
    if (!isTauriRuntime()) {
      return await apiRequest(`/api/providers/${appId}/${id}/switch`, {
        method: "POST",
      });
    }
...
export const universalProvidersApi = {
  ...
  async getAll(): Promise<UniversalProvidersMap> {
    if (!isTauriRuntime()) {
      return apiRequest("/api/universal-providers");
    }
  ...
  async get(id: string): Promise<UniversalProvider | null> {
    if (!isTauriRuntime()) {
      return apiRequest(`/api/universal-providers/${encodeURIComponent(id)}`);
    }
  ...
  async sync(id: string): Promise<boolean> {
    if (!isTauriRuntime()) {
      return apiRequest(`/api/universal-providers/${encodeURIComponent(id)}/sync`, {
        method: "POST",
      });
    }
